...
public class FichierController{
	...
	public ResponseEntity<String> createFile(@Valid @RequestBody FichierDto fichierDto, Authentication auth) {
		if (auth != null || hasRole(auth, Roles.PERSONNECONTACT)) {
	    	...
	    	try {
                ...
                String nomFichierUnique = generateUniqueFileName(fichierDto.getNomFichier());
                ...
                Fichier file = new Fichier(null, fichierDto.getNomFichier(), fichierDto.getTypeF(), fichierDto.getDate(), cheminStockage, contenu, persC);
    	...
    	else return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Le fichier existe déjà.");
	}
	...
	public ResponseEntity<Resource> downloadFile(@PathVariable String fileName) throws MalformedURLException {
	    if (fileName.isEmpty()) return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(null);
	    else {
	    	//try {
...
//	    	}
	...
	private void saveFileToServer(byte[] contenu, String cheminStockage) throws IOException {
        Path filePath = Paths.get(cheminStockage);
        Files.write(filePath, contenu);
	}

	    private String generateUniqueFileName(String originalFileName) {
	        // Générer un nom de fichier unique Exemple : "fichier12345.jpg"
	        String extension = FilenameUtils.getExtension(originalFileName);
	        String baseName = FilenameUtils.getBaseName(originalFileName);
	        String uniqueName = baseName + System.currentTimeMillis() + "." + extension;
	        return uniqueName;
	    }
}
